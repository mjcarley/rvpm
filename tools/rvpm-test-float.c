...
static void vortex_ring_func(gfloat r0, gfloat s, gfloat G,
			     gfloat *x, gfloat *f)

{
  gfloat r, z, th, r2, w, w0 ;
...
static void distribution_random(rvpm_distribution_t *d, gint np, gfloat s)

{
  ...
  for ( i = 0 ; i < np ; i ++ ) {
    ...
    *((gfloat *)rvpm_distribution_particle_radius(d,i)) = s ;
...

{
  ...
  for ( i = 0 ; i < rvpm_distribution_particle_number(d) ; i ++ ) {
    y = (gfloat *)rvpm_distribution_particle(d,i) ;
...
{
  gfloat *f, s, *work, *wt, x[3], omi[3], *om, *y, zeta ;
  gfloat r0, s0, sc, limits[6], h, tol, t0, al, r, gtol, dt ;
  char fname[64] ;
  grbf_workspace_t *ws ;
  ...
  grbf_grid_adjust_3d_f(&(limits[0]), &(limits[1]), &nx,
			    &(limits[2]), &(limits[3]), &ny,
			    &(limits[4]), &(limits[5]), &nz, h) ;
  work = (gfloat *)g_malloc0((nx*ny*nz*3 + wsize)*sizeof(gfloat)) ;
  wt   = (gfloat *)g_malloc0(nx*ny*nz*3*sizeof(gfloat)) ;
  ...
  ws = grbf_workspace_alloc_f(3, MAX(nx,MAX(ny,nz))) ;
  grbf_workspace_init_3d_f(ws, f, 3, nx, 3*ny*nz, ny, 3*nz, nz,
  ...
  grbf_interpolation_weights_fft_3d_f(f, 3, nx, 3*ny*nz, ny, 3*nz, nz, 3,
					  ws, wt, 3, 3*ny*nz, 3*nz) ;
  fprintf(stderr, "[%lg]\n", g_timer_elapsed(timer, NULL)-t0) ;

  np = grbf_grid_count_points_3d_f(wt, 3, nx, 3*ny*nz, ny, 3*nz, nz, 3,
				       gtol) ;
  ...
  for ( i = 0 ; i < nx ; i ++ ) {
	...
	if ( rvpm_vector_scalar(om,om) > gtol*gtol ) {
	  y = (gfloat *)rvpm_distribution_particle(d,n) ;
	  y[0] = limits[0] + h*i ;
	  y[1] = limits[2] + h*j ;
	  y[2] = limits[4] + h*k ;
  ...
  for ( i = 0 ; i < 100 ; i ++ ) {
    ...
    for ( j = 0 ; j < np ; j ++ ) {
      y = (gfloat *)rvpm_distribution_particle(d,j) ;
      /* r = sqrt(rvpm_vector_distance2(xi,y)) ; */
      r = sqrt(rvpm_vector_distance2(x,y)) ;
      om = (gfloat *)rvpm_distribution_vorticity(d,j) ;
      s = *((gfloat *)rvpm_distribution_particle_radius(d,j)) ;
  ...
  for ( i = 0 ; i < 200 ; i ++ ) {
    ...
    memset(work, 0, 3*np*sizeof(gfloat)) ;
    for ( j = 0 ; j < np ; j ++ ) {
      rvpm_vorticity_velocity_gradient_f(d, s,
					     RVPM_KERNEL_WINCKELMANS_LEONARD,
					     (gfloat *)rvpm_distribution_particle(d,j),
					     &(work[3*j]), NULL) ;
    }
    for ( j = 0 ; j < np ; j ++ ) {
      y = (gfloat *)rvpm_distribution_particle(d,j) ;
...
{
  gfloat *f, s, *work, *wt, x[3], *om, *y, zeta, reg ;
  gfloat r0, s0, sc, limits[6], h, tol, t0, al, gtol, uv[12], ut[12] ;
  gfloat err[12] = {0} ;
  grbf_workspace_t *ws ;
  rvpm_distribution_t *d ;
  rvpm_tree_t *tree ;
  ...
  for ( i = 0 ; i < 1000 ; i ++ ) {
    ...
    memset(uv, 0, 12*sizeof(gfloat)) ;
    rvpm_vorticity_velocity_gradient_f(d, reg,
					   kernel,
					   x, uv, &(uv[3])) ;
    fprintf(stdout, "%1.16e %1.16e %1.16e", x[0], x[1], x[2]) ;
    for ( j = 0 ; j < 12 ; j ++ ) fprintf(stdout, " %1.16e", uv[j]) ;
    memset(ut, 0, 12*sizeof(gfloat)) ;
    rvpm_tree_velocity_gradient_f(tree, reg,
				      kernel,
				      x, ut, &(ut[3]), work) ;
...
{
  gfloat s, *work, *wt, x[3], *om, *y, zeta, reg ;
  gfloat r0, s0, sc, limits[6], h, tol, t0, al, gtol, dt, *u ;
  char fname[64] ;
  grbf_workspace_t *ws ;
  rvpm_distribution_t *d ;
  rvpm_tree_t *tree ;
  ...
  fprintf(stderr, "allocating workspace of %lu bytes\n",
	  nx*ny*nz*3*sizeof(gfloat)) ;
  wt   = (gfloat *)g_malloc0(nx*ny*nz*3*sizeof(gfloat)) ;

  ws = grbf_workspace_alloc_f(3, MAX(nx,MAX(ny,nz))) ;
  fprintf(stderr, "initialising workspace\n") ;
  grbf_workspace_init_3d_f(ws, wt, 3, nx, 3*ny*nz, ny, 3*nz, nz,
			       wt, 3, 3*ny*nz, 3*nz, al, N, work) ;
  ...
  fprintf(stderr, "evaluating Gaussian weights (FFT) ") ;
  t0 = g_timer_elapsed(timer, NULL) ;
  grbf_interpolation_weights_fft_3d_f(wt, 3, nx, 3*ny*nz, ny, 3*nz, nz, 3,
					  ws, wt, 3, 3*ny*nz, 3*nz) ;
  fprintf(stderr, "[%lg]\n", g_timer_elapsed(timer, NULL)-t0) ;
  ...
  fprintf(stderr, "setting up FMM tree\n") ;
  tree = rvpm_tree_new_f(d, 4, order_max, work) ;
  ...
  /*time step for the crack*/
  u = (gfloat *)g_malloc0(ustr*np*sizeof(gfloat)) ;
  t0 = g_timer_elapsed(timer, NULL) ;
  for ( i = 0 ; i < 200 ; i ++ ) {
...

{
  ...
  memcpy(e->data, d->data, np*RVPM_DISTRIBUTION_PARTICLE_SIZE*sizeof(gfloat)) ;
  rvpm_distribution_particle_number(e) = np ;
  
  rvpm_distribution_interleaved_to_contiguous_f(e) ;  
  ...
  rvpm_distribution_contiguous_to_interleaved_f(e) ;
  for ( i = 0 ; i < np ; i ++ ) {
    /* x = &(d->data[i*RVPM_DISTRIBUTION_PARTICLE_SIZE]) ; */
    x = (gfloat *)rvpm_distribution_particle(d,i) ;
    for ( j = 0 ; j < RVPM_DISTRIBUTION_PARTICLE_SIZE ; j ++ ) {
...
{
  gfloat x[3], y[3], dy[3], x1[3], x2[3], K[12]={0}, K1[3], K2[3], ee, s ;
  gfloat err, w[3], du[9], u1[3], u2[3], cdu[9] ;
  rvpm_kernel_func_f_t kfunc ;
  gint i ;
  ...
  kfunc = rvpm_kernel_func_f(kernel) ;
...
{
  gfloat r, z, ur, uz, rmax, zmin, zmax ;
  gdouble data[4] ;
  gqr_rule_t *gs, *gth ;
  ...
  for ( i = 0 ; i <= nr ; i ++ ) {
    ...
    for ( j = 0 ; j <= nz ; j ++ ) {
      ...
      rvpm_stream_func_velocity_f(rvpm_stream_func_vorticity_gaussian, data,
...
static void stream_func(gfloat r, gfloat z,
			gfloat r1, gfloat z1,
			gfloat *G, gfloat *Gr, gfloat *Gz) 

{
  gfloat R1, R2, lm, lm_r, lm_z, K, E, dK, dE ;
...
{
  gfloat r, z, r1, z1, ee, G, Gr, Gz, G1, G2, gr, gz ;
...

{
  ...
  rvpm_solver_coefficients_f(s, a, b, &n, &p) ;
...
{
  gfloat g, f, x[3], y[3], err, errd, Kr[12], Kf[12], R, s ;
  gint i ;
  ...
  for ( R = 0 ; R < 7.5*s ; R += 0.01*s ) {
    ...
    rvpm_kernel_GS_f(x, y, s, Kr, &(Kr[3])) ;
    rvpm_kernel_fast_GS_f(x, y, s, Kf, &(Kf[3])) ;
